...
async def test_multi_chunk_renumbering() -> None:
    ...
    with patch("gemini_ocr.gemini_ocr._generate_markdown_for_chunk") as mock_gen_md:
        mock_gen_md.side_effect = [
            "Part 1: <!--table--> Content <!--table--> <!--figure-->",
            "Part 2: <!--table--> Content <!--figure--> <!--figure-->",
        ]

        # Mock document.chunks to return our dummy chunks
        with patch("gemini_ocr.document.chunks", return_value=["c1", "c2"]):
            # Mock docai_ocr.generate_bounding_boxes to avoid real calls
            with patch("gemini_ocr.docai_ocr.generate_bounding_boxes", return_value=[]):
                res = await gemini_ocr.process_document(
                    "dummy.pdf",
                    settings=settings.Settings(
                        project_id="test", location="us", ocr_processor_id="id", layout_processor_id="id"
                    ),
                )
