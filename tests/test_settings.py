...
import pytest
from gemini_ocr.settings import Settings, OcrMode
...
def test_settings_explicit_overrides_env():
    """Test that explicit arguments override environment variables."""
    env = {
        "GEMINI_OCR_PROJECT": "env-project",
    }
    with patch.dict(os.environ, env, clear=True):
        # We must provide other required fields that are not in env to avoid validation error
        # Or mock them in env as well
        with patch.dict(
            os.environ,
            {
